# -*- coding: utf-8 -*-
"""Event_Barrier_normalizationV2.ipynb
...
df = pd.read_csv('/content/drive/MyDrive/Practicum/incident_outputs/patricks_incident_files/flat_incidents_combinedV3.csv')
...
controls_df = pd.read_csv('/content/drive/MyDrive/Practicum/incident_outputs/patricks_incident_files/controls_combinedV3.csv')
...
ADMIN_FAMILIES = {
    ...
    "hazard_analysis_prework_checks": [
        ...
        "stop and think",
        "take 5 safety check",
        "pre job briefing risk review",
        "worksite risk assessment",
        "site risk assessment",
        "pre start safety check",
        "pre start up checklist",
        "energy isolation verification pre task"
    ],
    "training": [
        "training program",
        "safety training",
    ...
    "procedures": [
        ...
        "work instruction documented procedure",
        "written procedure compliance",
        "operating manual",
        "emergency procedure",
    ...
    "change_management": [
        ...
        "technical query approval process",
    ...
    "communication": [
        ...
        "permit communication between teams",
        "incident notification",
        "safety alert bulletin",
        "toolbox communication briefing"
    ],
    ...
    "operating_controls_and_limits": [
        ...
        "control room operating limits",
        "alarm set point management",
        "operating limit",
        "operating limits",
        ...
        "administrative safeguard operating limits",
        "alarm set point management",          # keep if useful
        "safe venting limit",
    ...
    "regulatory_and_permits": [
        ...
        "government inspection compliance"
    ]
}
...
def has_units(text: str) -> bool:
    # expand as needed
    return bool(re.search(r"\b(psig|psi|bar|barg|kpa|mpa|degc|°c|degf|°f|ppm|%|bbl|gpm|m3|scf|mscf|kg|lb)\b", (text or "").lower()))

def has_limit_phrases(text: str) -> bool:
    return bool(re.search(
        r"(do not exceed|not exceed|must not exceed|maximum allowable|minimum allowable|safe fill|operating envelope|setpoint|trip setpoint|high-high|low-low|upper limit|lower limit)",
        (text or "").lower()
    ))
...
for fam, phrases in ADMIN_FAMILIES.items():
    embs = model.encode(phrases, normalize_embeddings=True, show_progress_bar=False)
    centroid = embs.mean(axis=0)
    admin_centroids[fam] = centroid / np.linalg.norm(centroid)

VALID_ADMIN_FAMILIES = set(ADMIN_FAMILIES.keys()) | {"other_administrative", "unknown"}
...
def normalize_tokens(text: str, tokens):
    if not isinstance(tokens, list) or len(tokens) == 0:
        ...
        words = [str(t).lower() for t in tokens]
    return [stemmer.stem(w) for w in words]

def token_contains(tokens, root):
    return any(root in t for t in tokens)

def assign_admin_family(text_norm, tokens=None, min_sim=0.45):

    if not text_norm or str(text_norm).strip() == "":
        return "unknown"

    tokens = normalize_tokens(text_norm, tokens)
    ...
    if (
        ...
        or token_contains(tokens, "regulator")
        or token_contains(tokens, "complianc")
        or token_contains(tokens, "govern")
        or token_contains(tokens, "author")
        or token_contains(tokens, "licens")
    ):
        return "regulatory_and_permits"
    ...
    # -------------------------
    if (
        token_contains(tokens, "moc")
        or token_contains(tokens, "modif")
        or token_contains(tokens, "chang")
        or token_contains(tokens, "redesign")
        or token_contains(tokens, "temporari")
        or token_contains(tokens, "engin chang")
    ):
    ...
    # -------------------------
    training_terms = (
        token_contains(tokens, "train")
        or token_contains(tokens, "competenc")
        or token_contains(tokens, "certif")
        or token_contains(tokens, "qualif")
        or token_contains(tokens, "induct")
        or token_contains(tokens, "onboard")
        or token_contains(tokens, "refresher")
        or token_contains(tokens, "drill")
        or token_contains(tokens, "exercise")
        or token_contains(tokens, "simulat")
        or token_contains(tokens, "ojt")
        or token_contains(tokens, "cbt")
    )

    # avoid misrouting "training" used as a verb in weird contexts (rare)
    if training_terms:
        return "training"
    # -------------------------
    # 3) Hazard Analysis / Pre-work
    # -------------------------
    hazard_terms = (
        token_contains(tokens, "hazard")
        or token_contains(tokens, "risk")
        or token_contains(tokens, "hazop")
        or token_contains(tokens, "hazid")
        or token_contains(tokens, "jsa")
        or token_contains(tokens, "jha")
        or token_contains(tokens, "jra")
        or token_contains(tokens, "tra")
        or token_contains(tokens, "lmra")
    )

    prework_context = (
        token_contains(tokens, "pre")         # catches pre-job, pre-task, prework (stemming helps)
        or token_contains(tokens, "prior")
        or token_contains(tokens, "before")
        or token_contains(tokens, "toolbox")
        or token_contains(tokens, "brief")
        or token_contains(tokens, "walkdown")
        or token_contains(tokens, "checklist")
        or token_contains(tokens, "check")    # pre-start checks often just say "checks"
        or token_contains(tokens, "verify")
    )

    analysis_action = (
        token_contains(tokens, "assess")
        or token_contains(tokens, "analysi")
        or token_contains(tokens, "identif")
        or token_contains(tokens, "review")
        or token_contains(tokens, "evalu")
    )

    # Primary hazard analysis capture:
    if (
        # Classic acronyms always count
        token_contains(tokens, "hazop")
        or token_contains(tokens, "hazid")
        or token_contains(tokens, "jsa")
        or token_contains(tokens, "jha")
        or token_contains(tokens, "pssr")
        or token_contains(tokens, "simop")
        # Risk/hazard + (assessment/review/identify) and some prework signal
        or (hazard_terms and analysis_action and prework_context)
        # Common shorthand phrases like "risk assessment" even without explicit "pre"
        or (hazard_terms and analysis_action)
    ):
        return "hazard_analysis_prework_checks"

    # -------------------------
    # 4) Planning (pre-event)
    # -------------------------
    if (
        token_contains(tokens, "plan")
        and (
            token_contains(tokens, "hurrican")
            or token_contains(tokens, "weather")
            or token_contains(tokens, "storm")
            or token_contains(tokens, "emerg")
            or token_contains(tokens, "evacu")
            or token_contains(tokens, "continu")
        )
    ):
        return "planning"
    ...
    # -------------------------
    if (
        token_contains(tokens, "mainten")
        or token_contains(tokens, "pm")
        or token_contains(tokens, "servic")
        or token_contains(tokens, "repair")
        or token_contains(tokens, "overhaul")
        or token_contains(tokens, "work order")
    ):
        return "maintenance"

    # -------------------------
    # 5) Operating Controls & Limits (tightened)
    # -------------------------
    limit_context = (
        token_contains(tokens, "limit")
        or token_contains(tokens, "envelop")
        or token_contains(tokens, "setpoint")
        or token_contains(tokens, "threshold")
        or token_contains(tokens, "mawp")
        or token_contains(tokens, "sol")
    )

    parameter_context = (
        token_contains(tokens, "press")      # pressure / pressur*
        or token_contains(tokens, "temper")  # temperature
        or token_contains(tokens, "level")
        or token_contains(tokens, "flow")
        or token_contains(tokens, "speed")
        or token_contains(tokens, "vibrat")
        or token_contains(tokens, "ppm")
    )

    strong_limit_signal = (
        has_limit_phrases(text_norm)
        or has_units(text_norm)
        or has_number(text_norm)
    )

    # Only classify as operating controls if it really looks like a numeric/parameter constraint
    if limit_context and (strong_limit_signal or parameter_context):
        return "operating_controls_and_limits"


    # -------------------------
    # 7) Monitoring (expanded but controlled)
    # -------------------------

    monitor_terms = (
        token_contains(tokens, "monitor")
        or token_contains(tokens, "surveil")
        or token_contains(tokens, "trend")
        or token_contains(tokens, "review")
        or token_contains(tokens, "audit")
        or token_contains(tokens, "oversight")
        or token_contains(tokens, "oversee")
        or token_contains(tokens, "supervis")
        or token_contains(tokens, "firewatch")
    )

    detect_term = token_contains(tokens, "detect")

    third_party_context = (
        token_contains(tokens, "third") and token_contains(tokens, "part")
    )

    # Only treat "detect" as administrative monitoring if not clearly engineering
    engineering_context = (
        token_contains(tokens, "sensor")
        or token_contains(tokens, "detector")
        or token_contains(tokens, "transmitt")
    ...
    if (
        monitor_terms
        or (detect_term and not engineering_context)
        or (third_party_context and token_contains(tokens, "oversight"))
    ):
        return "monitoring"
    ...
    # -------------------------
    if (
        token_contains(tokens, "communic")
        or token_contains(tokens, "handover")
        or token_contains(tokens, "notif")
        or token_contains(tokens, "alert")
        or token_contains(tokens, "report")
        or token_contains(tokens, "brief")
    ):
    ...
    # -------------------------
    procedure_context = (
        token_contains(tokens, "procedur")
        or token_contains(tokens, "sop")
        or token_contains(tokens, "instruct")
        or token_contains(tokens, "manual")
        or token_contains(tokens, "step")
        or token_contains(tokens, "checklist")
    )

    # If it's a procedure BUT it also contains hard limit signals, keep it as operating controls
    if procedure_context:
        if limit_context and (has_limit_phrases(text_norm) or has_units(text_norm) or has_number(text_norm)):
            return "operating_controls_and_limits"
        return "procedures"
    ...
    scores = sorted(
        [(fam, float(v @ c)) for fam, c in admin_centroids.items()],
        key=lambda x: x[1],
        reverse=True
    )

    (best_fam, best_sim), (_, second_sim) = scores[0], scores[1]

    if best_sim >= min_sim and (best_sim - second_sim) >= 0.03:
        return best_fam

    return "other_administrative"

admin_df["admin_family"] = admin_df.apply(
    lambda r: assign_admin_family(
...
print(admin_df["admin_family"].value_counts(dropna=False))

def admin_centroid_best(text_norm: str):
    v = model.encode([text_norm], normalize_embeddings=True, show_progress_bar=False)[0]
    scores = sorted(
        [(fam, float(v @ c)) for fam, c in admin_centroids.items()],
        key=lambda x: x[1],
    ...
    return scores  # list of (fam, sim) descending


def second_pass_admin(text_norm: str, min_sim2: float = 0.37, margin2: float = 0.01) -> str:
    if not text_norm or str(text_norm).strip() == "":
        return "unknown"

    scores = admin_centroid_best(text_norm)
    (best_fam, best_sim), (_, second_sim) = scores[0], scores[1]

    # Lower bar in pass 2
    if best_sim >= min_sim2 and (best_sim - second_sim) >= margin2:
        return best_fam

    # If you want to be even more aggressive, you can drop margin entirely:
    # if best_sim >= min_sim2: return best_fam

    return "other_administrative"

mask_other = admin_df["admin_family"].eq("other_administrative")

# Run second pass only on current "other" rows
second_pass_results = admin_df.loc[mask_other, "family_match_text_norm"].apply(
    lambda t: second_pass_admin(t, min_sim2=0.25, margin2=0.01)
)

# Only update rows where pass2 produced a real family
update_mask = second_pass_results.ne("other_administrative")

admin_df.loc[mask_other, "admin_family"] = admin_df.loc[mask_other, "admin_family"].where(
    ~update_mask,
    second_pass_results
)

admin_df["admin_family"].value_counts(dropna=False)

import pandas as pd
pd.set_option('display.max_colwidth', None)
pd.set_option('display.max_rows', None)

admin_df[admin_df['admin_family']== 'other_administrative']['family_match_text_norm']

for family_name, group_df in admin_df.groupby('admin_family'):
    print(f"\n--- Admin Family: {family_name} ---")
    for text in group_df['family_match_text_norm']:
        print(text)
...
# -----------------------------
# 1) Create normalized engineering fields (use already existing raw columns)
# -----------------------------
engineering_df = engineering_df.copy()

# These columns should already exist from earlier steps and are correctly populated.
...
# -----------------------------
# 2) Define ENGINEERING family taxonomy (using ENG_FAMILIES_5 from previous cells)
# -----------------------------
ENGINEERING_BARRIER_FAMILIES = {
    "overpressurization_gas_discharge_gas_isolation": [
        # Overpressure / relief
        "pressure relief valve PSV PRV rupture disk overpressure protection",
        "blowdown depressurization system",
        "flare system vent stack controlled venting",
        # Isolation / shut-in
        "emergency shutdown ESD EIV SDV isolation valve shut in",
        "double block and bleed DBB isolation",
        "subsea isolation valve SIV",
        # Well control / high consequence isolation
        "blowout preventer BOP well control shut in"
    ],

    "fluid_discharge_and_containment": [
        "secondary containment bund berm dike sump drain spill containment",
        "double walled piping containment sleeve",
        "tank overfill protection high high level shutoff",
        "mechanical seal seal pot leak tight containment",
        "well casing cementing packer plug primary containment"
    ],

    "prevention_of_ignition": [
        "intrinsically safe instrumentation explosion proof electrical",
        "grounding bonding earthing static discharge control",
        "inert gas blanketing nitrogen purge inerting",
        "flame arrestor spark arrestor",
        "hazardous area classification ATEX IECEx ignition prevention"
    ],

    "detection_monitoring_alarms": [
        "gas detection H2S detector LEL monitor",
        "fire flame smoke detection system",
        "leak detection system",
        "alarm annunciator warning indicator",
        "pressure level temperature flow transmitter sensor monitoring SCADA"
    ],

    "mechanical_integrity": [
        "corrosion monitoring cathodic protection CP",
        "inspection NDT wall thickness ultrasonic testing",
        "pipeline coating protective coating lining",
        "flange gasket packing seal integrity",
        "piping vessel tank structural integrity fitness for service"
    ],
}
...
eng_anchor_texts = []
eng_lookup = []
for fam, phrases in ENGINEERING_BARRIER_FAMILIES.items():
    for p in phrases:
...
# -----------------------------
VALID_ENG_FAMILIES = set(ENGINEERING_BARRIER_FAMILIES.keys()) | {"other_engineering", "unknown"}
...
def assign_eng_family(text_norm: str, tokens: list[str], min_sim: float = 0.68) -> str:
    ...
    # -------------------------
    # High-precision RULES -> 5 families
    # -------------------------
    ...
    if (
        ...
        or token_contains(tokens, "scada")
        or token_contains(tokens, "transmitt")
        ...
        or token_contains(tokens, "lel")
    ):
        return "detection_monitoring_alarms"
# 5) Mechanical integrity
    if (
        token_contains(tokens, "corros")
        or token_contains(tokens, "cathod")
        or token_contains(tokens, "coating")
        or token_contains(tokens, "inspect")
        or token_contains(tokens, "ndt")
        or token_contains(tokens, "ultra")
        or token_contains(tokens, "thick")
        or token_contains(tokens, "integrit")
        or token_contains(tokens, "gasket")
        ...
        or token_contains(tokens, "vessel")
        or token_contains(tokens, "tank")
    ):
        return "mechanical_integrity"

    # 2) Prevention of ignition
    if (
        token_contains(tokens, "intrins")
        or token_contains(tokens, "explos")
        or token_contains(tokens, "atex")
        or token_contains(tokens, "iecex")
        or token_contains(tokens, "ground")
        or token_contains(tokens, "bond")
        or token_contains(tokens, "earth")
        or token_contains(tokens, "static")
        or token_contains(tokens, "spark")
        or token_contains(tokens, "flame")  # flame arrestor context (not detection)
        or token_contains(tokens, "inert")
        or token_contains(tokens, "nitrogen")
        or token_contains(tokens, "purge")
        or token_contains(tokens, "blanket")
    ):
        return "prevention_of_ignition"

    # put this BEFORE the overpressure/gas/isolation rule
    if (
        token_contains(tokens, "spill") or token_contains(tokens, "contain")
        or token_contains(tokens, "bund") or token_contains(tokens, "berm")
        or token_contains(tokens, "dike") or token_contains(tokens, "sump")
        or token_contains(tokens, "drain") or token_contains(tokens, "overfil")
        or token_contains(tokens, "tank") or token_contains(tokens, "bund")
        or token_contains(tokens, "hose") or token_contains(tokens, "transfer")
        or token_contains(tokens, "loading") or token_contains(tokens, "unloading")
        or token_contains(tokens, "seal") or token_contains(tokens, "pump")
        or token_contains(tokens, "liquid")
    ):
        return "fluid_discharge_and_containment"

    # 3) Overpressurization / gas discharge / gas isolation
    if (
        # isolation/shut-in
        ...
        # blowdown / vent / flare (gas discharge control)
        or token_contains(tokens, "blowdown")
        or token_contains(tokens, "depress")
        or token_contains(tokens, "vent")
        or token_contains(tokens, "flare")
        or token_contains(tokens, "stack")
    ):
        return "overpressurization_gas_discharge_gas_isolation"
...
engineering_df[engineering_df['eng_family'] == 'overpressurization_gas_discharge_gas_isolation']['family_match_text_norm']
#prettygood

engineering_df[engineering_df['eng_family'] == 'detection_monitoring_alarms']['family_match_text_norm']

engineering_df[engineering_df['eng_family'] == 'mechanical_integrity']['family_match_text_norm']

engineering_df[engineering_df['eng_family'] == 'fluid_discharge_and_containment']['family_match_text_norm']

engineering_df[engineering_df['eng_family'] == 'other_engineering']['family_match_text_norm']
#add this to mechanical integrity

engineering_df.loc[engineering_df['eng_family'] == 'other_engineering', 'eng_family'] = 'mechanical_integrity'

engineering_df[engineering_df['eng_family'] == 'prevention_of_ignition']['family_match_text_norm']
#add this to pressurization

engineering_df.loc[engineering_df['eng_family'] == 'prevention_of_ignition', 'eng_family'] = 'overpressurization_gas_discharge_gas_isolation'
