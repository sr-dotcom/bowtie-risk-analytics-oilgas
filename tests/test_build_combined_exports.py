...
def test_build_flat_incidents_reads_bom_json(tmp_path: Path) -> None:
    """Regression test: build_flat_incidents must correctly read utf-8-sig (BOM) encoded JSON."""
    from src.analytics.build_combined_exports import build_flat_incidents

    # Minimal V2.3 incident with one control and one PIF mentioned
    incident = {
        "incident_id": "bom_flat_001",
        "source": {"document_type": "test"},
        "context": {},
        "event": {
            "top_event": "Loss of Containment",
            "incident_type": "leak",
            "summary": "BOM encoding test incident.",
        },
        "bowtie": {
            "controls": [
                {
                    "control_id": "CF_1",
                    "name": "BOM Flat Control",
                    "side": "left",
                    "barrier_role": "prevent",
                    "barrier_type": "engineering",
                    "line_of_defense": 1,
                    ...
                    "performance": {"barrier_status": "failed", "barrier_failed": True},
                    "human": {
                        "human_contribution_value": "direct",
                        "barrier_failed_human": True,
                    },
                    "evidence": {"confidence": "high", "supporting_text": ["text"]},
                }
            ]
        },
        "pifs": {
            "people": {"competence": {"competence_mentioned": True}},
        },
        "notes": {"schema_version": "2.3"},
    }

    # Write with BOM encoding (utf-8-sig adds the byte order mark at file start)
    bom_file = tmp_path / "bom_flat_test.json"
    bom_file.write_text(json.dumps(incident), encoding="utf-8-sig")

    out_csv = tmp_path / "flat.csv"
    count = build_flat_incidents(tmp_path, out_csv)
    ...
    assert len(rows) == 1
    assert rows[0]["incident_id"] == "bom_flat_001"
