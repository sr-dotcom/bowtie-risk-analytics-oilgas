...
def test_flatten_reads_bom_encoded_json(tmp_path: Path) -> None:
    """Regression test: flatten_all must correctly read utf-8-sig (BOM) encoded JSON files."""
    import csv as _csv

    # Build a minimal valid V2.3 incident dict with one control using nested schema
    incident = {
        "incident_id": "bom_test_001",
        "source": {"document_type": "test"},
        "context": {},
        "event": {"top_event": "test"},
        "bowtie": {
            "controls": [
                {
                    "control_id": "C-BOM-01",
                    "name": "BOM Test Control",
                    "side": "prevention",
    ...
    # Write the JSON with a BOM (utf-8-sig encoding adds the byte order mark)
    schema_dir = tmp_path / "schema_v2_3"
    schema_dir.mkdir()
    bom_file = schema_dir / "bom_test.json"
    bom_file.write_text(json.dumps(incident), encoding="utf-8-sig")

    out_csv = tmp_path / "controls.csv"
    count = flatten_all(schema_dir, out_csv)
    ...
    assert len(rows) == 1
    assert rows[0]["incident_id"] == "bom_test_001"
    assert rows[0]["control_id"] == "C-BOM-01"
